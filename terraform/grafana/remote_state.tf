...
# Grafana is exposed through the prod API Gateway (path-based routing under
# /grafana/*), so it needs the prod API Gateway ID and VPC Link ID.
data "terraform_remote_state" "prod" {
  backend = "s3"

  config = {
    bucket = var.tf_state_bucket_name
    key    = "otp-provider-prod.tfstate"
    region = var.tf_state_bucket_region
  }
}
...
locals {
  ...
  prod_api_gateway_id          = data.terraform_remote_state.prod.outputs.prod_api_gateway_id
  prod_api_gateway_vpc_link_id = data.terraform_remote_state.prod.outputs.prod_api_gateway_vpc_link_id
}
